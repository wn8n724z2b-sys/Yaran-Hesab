const KEY='yaran_financial_v1';
const WORK_KEY='yaran_work_session_v1';
...
function getWorkSessionStart(){
 let data=null;try{data=JSON.parse(sessionStorage.getItem(WORK_KEY)||'null')}catch(e){}
 const today=dateKey();if(!data||data.date!==today||!data.start){data={date:today,start:new Date().toISOString()};try{sessionStorage.setItem(WORK_KEY,JSON.stringify(data))}catch(e){}}
 return new Date(data.start);
}
function renderWorkSession(){
 const ms=Math.max(0,Date.now()-workStart.getTime()),mins=Math.floor(ms/60000),h=Math.floor(mins/60),m=mins%60;
 const sec=Math.floor(ms/1000)%60;$('#workSessionValue').textContent=pad(h)+':'+pad(m)+':'+pad(sec);
 $('#workSession').title='شروع برنامه: '+formatTime(workStart);$('#clock').textContent=formatTime(new Date());
}
...
function applyTheme(){document.documentElement.setAttribute('data-theme',db.settings.theme==='dark'?'dark':'light');$('#themeToggle').textContent=db.settings.theme==='dark'?'☀':'☾';$('#themeToggle').title=db.settings.theme==='dark'?'تم روشن':'دارک تلگرامی'}
const pageMeta={dashboard:['داشبورد','نمای کلی فروشگاه'],pos:['فروش','صندوق سریع و بارکدخوان'],products:['کالاها','عکس، قیمت، بارکد و موجودی'],inventory:['انبار','کاردکس و گردش موجودی'],parties:['اشخاص','مشتریان و شرکت‌ها'],reports:['گزارش‌ها','فروش، گردش پول و گزارش مالی'],settings:['تنظیمات','ظاهر، چاپگر، بارکد و Backup']};
function go(page){$$('.page').forEach(function(e){e.classList.remove('active')});$('#page-'+page).classList.add('active');$$('.nav-item').forEach(function(e){e.classList.toggle('active',e.dataset.page===page)});$('#pageTitle').textContent=pageMeta[page][0];$('#pageSubtitle').textContent=pageMeta[page][1];$('.sidebar').classList.remove('open');if(page==='pos')setTimeout(function(){$('#barcodeInput').focus()},30)}
$$('.nav-item').forEach(function(b){b.onclick=function(){go(b.dataset.page)}});$$('[data-go]').forEach(function(b){b.onclick=function(){go(b.dataset.go)}});$('#newSaleTop').onclick=function(){go('pos')};
...
function renderDashboard(){
 ...
 const days=Array.from({length:7},function(_,i){let d=new Date();d.setHours(12,0,0,0);d.setDate(d.getDate()-(6-i));return d});
 const vals=days.map(function(d){return db.sales.filter(function(x){return dateKey(new Date(x.time))===dateKey(d)}).reduce(function(a,x){return a+x.total},0)}),max=Math.max.apply(null,vals.concat([1]));
 $('#salesBars').innerHTML=days.map(function(d,i){return '<div class="bar-col"><small class="bar-value">'+(moneyVisible?money(vals[i]):'••••')+'</small><div class="bar" title="'+(moneyVisible?money(vals[i]):'پنهان')+'" style="height:'+Math.max(8,(vals[i]/max)*154)+'px"></div><small>'+DAY_NAMES[d.getDay()]+'</small></div>'}).join('');
...
function productImageHtml(p,cls){return p.image?'<img class="'+cls+'" src="'+esc(p.image)+'" alt="">':'<span class="'+cls+' placeholder">'+esc((p.name||'Y').trim().charAt(0)||'Y')+'</span>'}
...
function renderProducts(){
 ...
 $('#quickProducts').innerHTML=db.products.slice(0,8).map(function(p){return '<button class="quick-product" data-id="'+p.id+'">'+(p.image?'<img src="'+esc(p.image)+'" alt="">':'<span class="mini-placeholder">'+esc((p.name||'Y').charAt(0))+'</span>')+'<span><b>'+esc(p.name)+'</b><small>'+productPriceText(p)+' · موجودی '+num(p.stock)+' '+esc(p.baseUnit)+'</small></span></button>'}).join('');
 $$('.quick-product').forEach(function(b){b.onclick=function(){addToCart(b.dataset.id)}});
}
function renderInventory(){$('#invProductCount').textContent=num(db.products.length);$('#invUnits').textContent=num(db.products.reduce(function(a,p){return a+p.stock},0));$('#invCost').textContent=money(currentInventoryValue());$('#invLow').textContent=num(db.products.filter(function(p){return p.stock<=p.min}).length);$('#inventoryBody').innerHTML=db.inventory.slice().reverse().slice(0,100).map(function(x){return '<tr><td>'+formatDateTime(x.time)+'</td><td>'+esc(x.productName)+'</td><td>'+esc(x.type)+'</td><td dir="ltr"><b>'+(x.qty>0?'+':'')+num(x.qty)+'</b></td><td>'+esc(x.ref||'')+'</td></tr>'}).join('')||'<tr><td colspan="5" class="muted">گردش موجودی بعد از اولین فروش نمایش داده می‌شود.</td></tr>'}
function renderCustomers(){
 const selected=$('#customerSelect').value;$('#customerSelect').innerHTML=db.customers.map(function(c){return '<option value="'+c.id+'">'+esc(c.name)+'</option>'}).join('');if(selected&&db.customers.some(function(c){return c.id===selected}))$('#customerSelect').value=selected;
...
function renderReports(){
 ...
 $('#salesBody').innerHTML=list.slice().reverse().map(function(s){const c=db.customers.find(function(x){return x.id===s.customerId});return '<tr><td>#'+num(s.no)+(s.editedAt?' <span class="edited-dot" title="ویرایش شده">•</span>':'')+'</td><td>'+formatDateTime(s.time)+'</td><td>'+esc(c?c.name:'—')+'</td><td>'+esc(s.payment)+'</td><td>'+num(s.items.reduce(function(a,i){return a+i.qty},0))+'</td><td><b>'+money(s.total)+'</b></td><td><div class="invoice-actions"><button class="tiny-btn edit-invoice" data-sale="'+s.id+'">ویرایش</button><button class="tiny-btn danger delete-invoice" data-sale="'+s.id+'">حذف</button></div></td></tr>'}).join('')||'<tr><td colspan="7" class="muted">در این بازه فروشی ثبت نشده است.</td></tr>';bindInvoiceActions();
}
function netCashForDay(key){
 const cashSale=db.sales.filter(function(s){return s.payment==='نقدی'&&dateKey(new Date(s.time))===key}).reduce(function(a,s){return a+s.total},0),receipts=db.customerReceipts.filter(function(x){return dateKey(new Date(x.time))===key}).reduce(function(a,x){return a+x.amount},0),expenses=db.expenses.filter(function(x){return dateKey(new Date(x.time))===key}).reduce(function(a,x){return a+x.amount},0),supplier=db.supplierPayments.filter(function(x){return dateKey(new Date(x.time))===key}).reduce(function(a,x){return a+x.amount},0);return cashSale+receipts-expenses-supplier;
}
function renderCashFlowMonth(){const now=new Date(),daysCount=new Date(now.getFullYear(),now.getMonth()+1,0).getDate(),vals=[];for(let day=1;day<=daysCount;day++){const key=now.getFullYear()+'-'+pad(now.getMonth()+1)+'-'+pad(day);vals.push(netCashForDay(key))}const max=Math.max.apply(null,vals.map(Math.abs).concat([1]));$('#cashFlowBars').innerHTML=vals.map(function(v,i){return '<div class="bar-col"><small class="bar-value">'+(v?money(v):'')+'</small><div class="bar '+(v<0?'negative':'')+'" title="روز '+num(i+1)+': '+money(v)+'" style="height:'+Math.max(6,(Math.abs(v)/max)*145)+'px"></div><small>'+num(i+1)+'</small></div>'}).join('')}
function renderBrandIdentity(){const mark=$('.brand-mark'),logo=db.settings.storeLogo||'';mark.classList.toggle('has-logo',!!logo);mark.innerHTML=logo?'<img src="'+esc(logo)+'" alt="لوگو">':'Y';const prev=$('#storeLogoPreview');if(prev)prev.innerHTML=logo?'<img src="'+esc(logo)+'" alt="لوگو">':'Y'}
...
function renderSettings(){const s=db.settings;$('#storeName').value=s.storeName;$('#storeAddress').value=s.storeAddress||'';$('#storePhone').value=s.storePhone;$('#receiptFooter').value=s.receiptFooter;$('#printerName').value=s.printerName;$('#autoPrint').checked=!!s.autoPrint;$('#autoCut').checked=!!s.autoCut;$('#themeSelect').value=s.theme||'light';$('#scannerSuffix').value=s.scannerSuffix||'Enter';$('#autoBackupPerDay').value=String(s.autoBackupPerDay||0);renderBrandIdentity();renderAutoBackupStatus();renderDataSupportAccess();applyTheme()}
...
function openCategoryManager(){
 openModal('دسته‌بندی کالا','<form id="categoryForm" class="modal-form"><label class="field"><span>دسته‌بندی جدید</span><div style="display:flex;gap:7px"><input name="name" placeholder="مثلاً لبنیات" style="flex:1" required><button class="primary-btn">افزودن</button></div></label><div id="categoryChipList" class="category-chip-list"></div></form>');
 function redraw(){const box=$('#categoryChipList');box.innerHTML=db.categories.map(function(c){const used=db.products.filter(function(p){return p.categoryId===c.id}).length;return '<span class="category-chip"><b>'+esc(c.name)+'</b><small>'+num(used)+'</small>'+(c.id!=='cat-general'?'<button type="button" data-cat="'+c.id+'" title="حذف">×</button>':'')+'</span>'}).join('');$$('[data-cat]').forEach(function(b){b.onclick=function(){const cid=b.dataset.cat,cat=db.categories.find(function(c){return c.id===cid}),usedCount=db.products.filter(function(p){return p.categoryId===cid}).length;if(!cat)return;openModal('حذف دسته‌بندی','<div class="confirm-card danger-confirm"><div class="confirm-icon">!</div><h4>دسته «'+esc(cat.name)+'» حذف شود؟</h4><p>'+(usedCount?('این دسته روی '+num(usedCount)+' کالا استفاده شده است. با حذف دسته، این کالاها به دسته «عمومی» منتقل می‌شوند.'):'پس از تأیید، این دسته‌بندی حذف می‌شود.')+'</p><div class="modal-actions"><button id="cancelDeleteCategory" class="ghost-btn">انصراف</button><button id="confirmDeleteCategory" class="danger-btn">تأیید و حذف</button></div></div>');$('#cancelDeleteCategory').onclick=closeModal;$('#confirmDeleteCategory').onclick=function(){db.products.forEach(function(p){if(p.categoryId===cid)p.categoryId='cat-general'});db.categories=db.categories.filter(function(c){return c.id!==cid});if(save()){closeModal();toast('دسته «'+cat.name+'» حذف شد');renderProducts();renderCategoryOptions();}}}})}redraw();$('#categoryForm').onsubmit=function(e){e.preventDefault();const f=new FormData(e.target),name=String(f.get('name')||'').trim();if(!name)return;if(db.categories.some(function(c){return normalizeText(c.name)===normalizeText(name)})){toast('این دسته‌بندی قبلاً وجود دارد');return}db.categories.push({id:id('cat'),name:name});save();e.target.reset();redraw()}
}
...
let scanBuffer='',scanLast=0;document.addEventListener('keydown',function(e){if(['INPUT','TEXTAREA','SELECT'].indexOf(document.activeElement&&document.activeElement.tagName)>-1)return;const now=performance.now();if(now-scanLast>80)scanBuffer='';scanLast=now;const suffix=db.settings.scannerSuffix||'Enter';if(e.key===suffix||e.key==='Enter'){if(scanBuffer.length>=5){$('#lastScan').textContent='آخرین اسکن: '+scanBuffer;const b=scanBuffer;scanBuffer='';if(!$('#page-pos').classList.contains('active'))go('pos');findAndAdd(b)}return}if(e.key.length===1)scanBuffer+=e.key});
