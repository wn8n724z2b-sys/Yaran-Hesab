...
function askConfirm(message,opts){
 opts=opts||{};const wrap=$('#appDialog'),title=$('#appDialogTitle'),msg=$('#appDialogMessage'),ok=$('#appDialogConfirm'),cancel=$('#appDialogCancel'),icon=$('#appDialogIcon');
 if(!wrap)return Promise.resolve(false);
 if(appDialogResolver){appDialogResolver(false);appDialogResolver=null}
 title.textContent=opts.title||'تأیید عملیات';msg.textContent=String(message||'');ok.textContent=opts.confirmText||'تأیید';cancel.textContent=opts.cancelText||'انصراف';icon.textContent=opts.icon||'؟';
 wrap.className='app-dialog '+(opts.tone||'');wrap.classList.remove('hidden');
 return new Promise(function(resolve){appDialogResolver=resolve;setTimeout(function(){ok.focus()},20)});
}
function closeAppDialog(result){const wrap=$('#appDialog');if(wrap)wrap.classList.add('hidden');const r=appDialogResolver;appDialogResolver=null;if(r)r(!!result)}
if($('#appDialogConfirm'))$('#appDialogConfirm').onclick=function(){closeAppDialog(true)};
if($('#appDialogCancel'))$('#appDialogCancel').onclick=function(){closeAppDialog(false)};
if($('#appDialog'))$('#appDialog').onclick=function(e){if(e.target===this)closeAppDialog(false)};
document.addEventListener('keydown',function(e){if(e.key==='Escape'&&$('#appDialog')&&!$('#appDialog').classList.contains('hidden'))closeAppDialog(false)});
...
function renderWorkSession(){
 ...
 $('#workSession').title='شروع اولین کار امروز: '+formatTime(workStart)+' · از باز و بسته شدن برنامه مستقل است';$('#clock').textContent=formatTime(new Date());if($('#topDateText'))$('#topDateText').textContent=DAY_NAMES[new Date().getDay()]+' · '+formatDate(new Date());
}
...
if($('#quickAddProduct'))$('#quickAddProduct').onclick=function(){go('products');setTimeout(function(){if($('#addProductBtn'))$('#addProductBtn').click()},50)};if($('#quickAddCustomer'))$('#quickAddCustomer').onclick=function(){go('parties');setTimeout(function(){if($('#addCustomerBtn'))$('#addCustomerBtn').click()},50)};
...
function renderDashboard(){
 ...
 $('#mNetProfit').textContent=maskedMoney(netProfitPeriod('today'));$('#mCustomerDebt').textContent=maskedMoney(customerDebt());
 if($('#mIncome'))$('#mIncome').textContent=maskedMoney(cashIn('today'));if($('#mExpensesToday'))$('#mExpensesToday').textContent=maskedMoney(totalAmounts(periodExpenses('today')));
 $('#mInventoryValue').textContent=maskedMoney(currentInventoryValue());const unknownInv=unknownInventoryCostCount();$('#mInventorySnapshot').textContent=(unknownInv?num(unknownInv)+' کالا با بهای نامشخص · ':'')+'ارزش شناخته‌شده در '+formatDate(new Date());
 ...
 if($('#dashInventoryCount'))$('#dashInventoryCount').textContent=num(db.products.length);if($('#dashCustomerCount'))$('#dashCustomerCount').textContent=num(Math.max(0,db.customers.length-1));
 renderFinanceChart();
 const rs=$('#recentSales');if(rs)rs.innerHTML=db.sales.slice().reverse().slice(0,5).map(function(x){const c=db.customers.find(function(y){return y.id===x.customerId});const qty=(x.items||[]).reduce(function(a,i){return a+(Number(i.qty)||0)},0);return '<div class="recent-sale-item"><span class="recent-sale-icon"><svg viewBox="0 0 24 24"><path d="M3 5h2l2 10h10l2-7H6"/><circle cx="9" cy="19" r="1.5"/><circle cx="17" cy="19" r="1.5"/></svg></span><div class="recent-sale-copy"><b>'+(c?esc(c.name):'مشتری عمومی')+'</b><small>#'+num(x.no)+' · '+formatDateTime(x.time)+' · '+num(qty)+' قلم</small></div><div class="recent-sale-amount"><b>'+(moneyVisible?money(x.total):'•••••• ؋')+'</b><small>'+esc(x.payment)+'</small><div class="recent-sale-actions"><button class="edit-invoice" data-sale="'+x.id+'">ویرایش</button><button class="delete-invoice" data-sale="'+x.id+'">حذف</button></div></div></div>'}).join('')||'<div class="empty"><strong>هنوز فروشی ثبت نشده است.</strong><span>اولین فروش شما اینجا نمایش داده می‌شود.</span></div>';bindInvoiceActions();
}
...
function openCategoryManager(){
 ...
 function redraw(){const box=$('#categoryChipList');box.innerHTML=db.categories.map(function(c,i){const used=db.products.filter(function(p){return p.categoryId===c.id}).length;return '<div class="category-manage-row"><span class="category-drag-index">'+num(i+1)+'</span><input class="category-edit-input" data-edit-cat="'+c.id+'" value="'+esc(c.name)+'" '+(c.id==='cat-general'?'disabled':'')+'><small>'+num(used)+' کالا</small><div class="category-order-actions">'+(i>1|| (i>0&&c.id!=='cat-general')?'<button type="button" class="tiny-btn cat-up" data-id="'+c.id+'">↑</button>':'')+(i<db.categories.length-1&&c.id!=='cat-general'?'<button type="button" class="tiny-btn cat-down" data-id="'+c.id+'">↓</button>':'')+(c.id!=='cat-general'?'<button type="button" class="tiny-btn danger cat-delete" data-id="'+c.id+'">حذف</button>':'')+'</div></div>'}).join('');
  ...
  $$('.cat-delete').forEach(function(b){b.onclick=async function(){const cid=b.dataset.id,cat=db.categories.find(function(c){return c.id===cid}),usedCount=db.products.filter(function(p){return p.categoryId===cid}).length;if(!cat)return;if(!await askConfirm('دسته «'+cat.name+'» حذف شود؟'+(usedCount?'\n'+num(usedCount)+' کالا به دسته عمومی منتقل می‌شود.':''),{title:'حذف دسته‌بندی',confirmText:'حذف دسته',tone:'danger',icon:'×'}))return;db.products.forEach(function(p){if(p.categoryId===cid)p.categoryId='cat-general'});db.categories=db.categories.filter(function(c){return c.id!==cid});save();redraw();toast('دسته حذف شد')}})
 }
...
$('#restoreAutoBackup').onclick=async function(){
 if(!await askConfirm('اطلاعات فعلی با آخرین پشتیبان جایگزین می‌شود.',{title:'بازیابی پشتیبان',confirmText:'بازیابی',tone:'warning',icon:'↺'}))return;
...
if($('#loadTrainingDemo'))$('#loadTrainingDemo').onclick=async function(){
 if(!await askConfirm('ابتدا از اطلاعات فعلی پشتیبان گرفته می‌شود؛ سپس داده‌های آموزشی ۲ ماه و ۱۰ روزه جایگزین اطلاعات فعلی می‌شوند.',{title:'فعال‌سازی نسخه آموزشی',confirmText:'فعال‌سازی',tone:'warning',icon:'▶'}))return;
...
$('#resetDemo').onclick=async function(){if(await askConfirm('تمام اطلاعات برنامه به حالت اولیه برمی‌گردد. این کار قابل برگشت نیست.',{title:'بازنشانی اطلاعات',confirmText:'بازنشانی',tone:'danger',icon:'!'})){db=normalizeDB(clone(DEFAULT));moneyVisible=true;save();toast('حسابداری آسان به حالت اولیه بازنشانی شد')}};
...
function markScannerDetected(code){scannerDetectedAt=Date.now();if($('#scannerStatusPill')){$('#scannerStatusPill').textContent='بارکدخوان فعال';$('#scannerStatusPill').className='pill success'}if($('#scannerStatusTitle'))$('#scannerStatusTitle').textContent='بارکدخوان فعال است';if($('#scannerStatusHint'))$('#scannerStatusHint').textContent='اسکن سریع USB HID شناسایی شد.';if($('#lastScan'))$('#lastScan').textContent='آخرین اسکن: '+code+' · '+formatTime(new Date())}
function refreshScannerStatus(){if(!$('#scannerStatusPill'))return;if(!scannerDetectedAt){$('#scannerStatusPill').textContent='منتظر اسکن';$('#scannerStatusPill').className='pill warning';return}if(Date.now()-scannerDetectedAt>300000){$('#scannerStatusPill').textContent='وضعیت نامشخص';$('#scannerStatusPill').className='pill warning';if($('#scannerStatusHint'))$('#scannerStatusHint').textContent='در 5 دقیقه اخیر اسکن دریافت نشده؛ برای بررسی یک بارکد اسکن کنید.'}}
...
(async function setupCloseProtection(){try{if(window.__TAURI__&&window.__TAURI__.window&&window.__TAURI__.window.getCurrentWindow){const win=window.__TAURI__.window.getCurrentWindow();await win.onCloseRequested(async function(e){e.preventDefault();if(!await askConfirm('قبل از خروج، یک پشتیبان سبک از دیتابیس گرفته می‌شود.',{title:'بستن حسابداری آسان',confirmText:'بستن برنامه',tone:'warning',icon:'↗'}))return;try{await createAutoBackup(false)}catch(ignore){}await win.destroy()})}}catch(e){console.warn('close protection unavailable',e)}})();
