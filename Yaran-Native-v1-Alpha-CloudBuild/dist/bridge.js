(function () {
  ...
  async function hydrate() {
    ...
    try {
      ...
      window.__ASAN_PRELOADED_STATE__ = raw || null;
      const health = await invoke('native_health');
      window.__ASAN_NATIVE_HEALTH__ = health;
  ...
  function persist(raw) {
    ...
    return queueWrite(async function () {
      try {
        ...
        window.__ASAN_PRELOADED_STATE__ = raw;
        return true;
      } catch (error) {
  ...
  async function changeAdminPassword(password) {
    if (!hasTauri) return false;
    await invoke('change_admin_password', { password: String(password || '') });
    return true;
  }
